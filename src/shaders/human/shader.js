...
//-----------------------------------------------------------//
    ...
    color += vec3(snoise(random2(vUv)) * 0.06);
   // color = mix(vec3(1.),color,u_saturation);
...
//---------------blend with texture--------------------------------------------//
   ...
   vec4 CTexture = texture2D(tOne, vUv);
   vec4 C = vec4(color,CTexture.r);



   //contrast
   C = (C - 0.5) * u_contrast + vec4(0.5); 

   //saturation
   C.rgb = mix(vec3(1.),C.rgb,u_saturation); 
...
//-----------------------------------------------------------//
    ...
    //saturation
    color = mix(vec3(0.12),color,u_saturation*0.6); 

    gl_FragColor= vec4(color,CTexture.a);
}
`
export {vertex_human,fragment_human,vertex_shadow,fragment_shadow};
